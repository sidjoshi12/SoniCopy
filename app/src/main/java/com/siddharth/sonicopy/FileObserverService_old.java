...
public class FileObserverService_old extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //Images
        mImagesObserver = new FileObserver(intent.getStringExtra(mSource+ File.separator + "Media/Whatsapp Images")) {
            @Override
        ...
        //Videos
        mVideoObserver = new FileObserver(intent.getStringExtra(mSource+ File.separator + "Media/WhatsApp Video")) {
            @Override
            public void onEvent(int event, String path) {
                // If an event happens we can do stuff inside here
                // for example we can send a broadcast message with the event-id
